...
const ProfilePage = () => {
  ...
  const { user } = useAuth(); 
  const { updateUserDetails, fetchUserData } = useAuxRequests();
    ...
    const refetchUserDetails = async () => {
      try {
        ...
        if (updatedUser) {
          ...
          setFirstName(updatedUser.user_first_name);
          setLastName(updatedUser.user_last_name);
        }
      } catch (error) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (submittedUsername !== user?.user_name || submittedEmail !== user?.user_email || submittedFirstName !== user?.user_first_name || submittedLastName !== user?.user_last_name) {
      try {
  ...
  return (
    <div className="main-profile-container">
      ...
      <div className="profile-page-titles">
        <h2 className="profile-page-titles-main-title">Profile</h2>
        <h4 className="profile-page-titles-sub-title">The AllInOne User Profile Page</h4>
      </div>
      ...
      <div className="profile-page">
        <div className="profile-user-details-container">
          ...
          <div className="user-details-twodisplay-container">
            <div className="user-details-display-container">
              <h1 className="user-name">{user.user_name}</h1>
              <h4 className="first-and-last">{user.user_first_name} {user.user_last_name}</h4>
              <h4 className="email">{user.user_email}</h4>
              <button
                id="edit-button"
                type="button"
              ...
              >
                {isEditMode ? "Cancel" : "Edit User Details"}
              </button>
            </div>
